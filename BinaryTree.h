...
BinaryTree::size(const BinaryNode * subtree)
{
	...
	{
		count += size(subtree->left_);
	}

	if (subtree->right_ != NULL)
	{
		count += size(subtree->right_);
	}

	return count;
}

//L8P2 Modify this function
long
BinaryTree::height(const BinaryNode * subtree)
{
	...
	if (subtree->left_ != NULL)
	{
		sizeleft += 1+height(subtree->left_);
	}

	if (subtree->right_ != NULL)
	{
		sizeright += 1+height(subtree->right_);
	}
...
BinaryTree::leaves(const BinaryNode * subtree)
{
	...
	else
	{
		if (subtree->left_ != NULL)
		{
			count += leaves(subtree->left_);
		}
		if (subtree->right_ != NULL)
		{
			count += leaves(subtree->right_);
		}
	}
	return count;
}

//L8P4 Modify this function
short
